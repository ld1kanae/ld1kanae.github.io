...
(()=>{
  ...
  async function deleteSession(){const db=await openDb();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,"readwrite");tx.objectStore(STORE).delete(sessionId);tx.oncomplete=()=>{db.close();resolve()};tx.onerror=()=>{db.close();reject(tx.error)}})}
  ...
  function inheritedToken(){try{const w=JSON.parse(window.name||"{}").dmSongPublisher||{};return w.sessionId===sessionId&&/^github_pat_/.test(w.token||"")?w.token:""}catch{return ""}}
  async function maybeAutofill(input){input.name="password";input.autocomplete="current-password";const inherited=inheritedToken();if(inherited){input.value=inherited;return}if(!navigator.credentials?.get||typeof PasswordCredential!=="function")return;try{const c=await navigator.credentials.get({password:true,mediation:"optional"});if(/^github_pat_/.test(c?.password||""))input.value=c.password}catch{}}
  ...
  async function install(){
    const session=await getSession();if(!session||session.id!==songId)return;
