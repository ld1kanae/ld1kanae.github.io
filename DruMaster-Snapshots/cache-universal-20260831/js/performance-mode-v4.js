...
(()=>{
  ...
  const MIC_SELF_AUDIO_GUARD_MS=110,NOISE_CAPTURE_SEC=1.7,PAD_SAMPLE_TARGET=8;
  ...
  let runMode="normal",micStream=null,micSource=null,micFilter=null,rawAnalyser=null,rawTimeData=null,
      ...
      micSelfGuardUntil=0,padModel=null,padClassifier={lastScore:0,threshold:.62,lastAccepted:false,samples:0,testHits:0,testCandidates:0};
  ...
  function showPadSnareFeedback(){
    ...
    try{aecNode?.port.postMessage({type:"freeze",ms:190});aecNode?.port.postMessage({type:"suppressCandidates",ms:165})}catch{}
  ...
  async function ensureMic(){
    ...
    if(!workletLoaded){if(!ac.audioWorklet)throw Error("このブラウザでは精密な音響相殺を利用できません");await ac.audioWorklet.addModule("js/acoustic-cancel-processor.js?v=20260827-fingerprint1");workletLoaded=true}
  ...
  function bandPowerFromSpectrum(pow,sr){
    const hz=sr/512;return BAND_EDGES.slice(0,-1).map((lo,b)=>{const hi=BAND_EDGES[b+1],a=Math.max(1,Math.floor(lo/hz)),z=Math.min(pow.length-1,Math.ceil(hi/hz));let s=0,n=0;for(let i=a;i<=z;i++){s+=pow[i];n++}return n?s/n:0});
  }
  function aggregateBandPower(pcm,sr,start=0){
    const frames=[],hop=256,n=512,end=Math.min(pcm.length,start+Math.round(sr*.115));for(let at=start;at+512<=end;at+=hop)frames.push(bandPowerFromSpectrum(fftPower(pcm.subarray(at,at+512)),sr));
  ...
  function fingerprintFromPCM(pcm,sr,noise){
    ...
    const early=rmsRange(pcm,onset,onset+Math.round(sr*.025)),late=rmsRange(pcm,onset+Math.round(sr*.055),onset+Math.round(sr*.115)),rms=rmsRange(pcm,onset,Math.min(pcm.length,onset+Math.round(sr*.12)));
    let zc=0,last=pcm[onset]||0,end=Math.min(pcm.length,onset+Math.round(sr*.09));for(let i=onset+1;i<end;i++){const x=pcm[i];if((x>=0)!=(last>=0))zc++;last=x}
  ...
  function modelScore(fp){
    if(!padModel?.samples?.length)return 0;const scores=padModel.samples.map(s=>fingerprintSimilarity(fp,s)).sort((a,b)=>b-a),n=Math.min(3,scores.length);return scores.slice(0,n).reduce((s,x)=>s+x,0)/n;
  }
  ...
  function addModelSample(fp,limit=24){
    if(!padModel)padModel={samples:[],threshold:.62};
    padModel.samples.push(fp);if(padModel.samples.length>limit)padModel.samples.splice(1,padModel.samples.length-limit);recomputeModelThreshold();
  }
  ...
  function ensureCalibrationScreen(){
    ...
    section.innerHTML=`<div class="mic-calibration-card"><p class="mic-cal-eyebrow">PAD PRACTICE</p><h2>パッド音を登録</h2><p id="micCalInstruction" class="mic-cal-instruction">準備しています…</p><div class="mic-fingerprint-grid"><div class="mic-fingerprint-pane" data-fp-step="noise"><span>1</span><b>環境ノイズ</b><em id="fpNoiseState">待機</em><small>部屋の音をPCM収録し、周波数ごとのノイズ成分を記録します</small></div><div class="mic-fingerprint-pane" data-fp-step="sample"><span>2</span><b>パッド音登録</b><em id="fpSampleState">0 / ${PAD_SAMPLE_TARGET}</em><small>強弱を混ぜて普通に叩いてください。打音の音色を短い波形として登録します</small></div><div class="mic-fingerprint-pane mic-fingerprint-test" data-fp-step="test"><span>3</span><b>叩いてみてください</b><em id="fpTestState">未調整</em><small id="fpTestScore">登録後、ここで実際の認識を確認できます</small><i class="fp-hit-ring"></i></div></div><div class="mic-cal-meter"><i id="micCalMeterFill"></i></div><p id="micCalDetail" class="mic-cal-detail">環境音と打音が混ざることを前提に、環境ノイズ成分を差し引いて音色を比較します。</p><div class="mic-cal-actions"><button id="micCalAction" type="button">環境ノイズを収録</button><button id="micCalStart" type="button" class="hidden">演奏開始</button><button id="micCalRetry" type="button">最初からやり直す</button></div></div>`;
  ...
  function resetCalibration(){
    calibrationToken++;calBusy=false;calStage="noise";micCalibration=null;padModel={samples:[],threshold:.62};padClassifier={lastScore:0,threshold:.62,lastAccepted:false,samples:0,testHits:0,testCandidates:0};
    setCandidateMode("off");const s=ensureCalibrationScreen();s.querySelector("#fpNoiseState").textContent="待機";s.querySelector("#fpSampleState").textContent=`0 / ${PAD_SAMPLE_TARGET}`;s.querySelector("#fpTestState").textContent="未調整";s.querySelector("#fpTestScore").textContent="登録後、ここで実際の認識を確認できます";s.querySelector("#micCalStart").classList.add("hidden");s.querySelector("#micCalAction").classList.remove("hidden");s.querySelector("#micCalAction").disabled=false;s.querySelector("#micCalAction").textContent="環境ノイズを収録";setPane("noise","active");setInstruction("まず環境ノイズを収録します。パッドはまだ叩かないでください");setDetail("この録音は音量閾値ではなく、パッド音から背景成分を分離するためのノイズプロファイルとして使用します。");updateMeter(0);
  }
  ...
  async function recordNoise(){
    if(calBusy)return;calBusy=true;const token=calibrationToken,b=ui("#micCalAction");b.disabled=true;b.textContent="収録中…";ui("#fpNoiseState").textContent="収録中";setCandidateMode("off");setInstruction("約2秒、そのままにしてください");
    try{
      const capturePromise=beginAECCapture(NOISE_CAPTURE_SEC),until=performance.now()+NOISE_CAPTURE_SEC*1000;while(token===calibrationToken&&performance.now()<until){updateMeter(readRawLevel().peak);await waitFrame()}
      const capture=await capturePromise;if(token!==calibrationToken)return;const pcm=new Float32Array(capture.mic);micCalibration={noise:noiseProfileFromPCM(pcm,capture.sampleRate||ac.sampleRate),aec:null};ui("#fpNoiseState").textContent="完了";
      calStage="sample";setPane("sample","active");setCandidateModeForStage();b.disabled=false;b.textContent="パッド音の登録を開始";setInstruction(`次にパッドを${PAD_SAMPLE_TARGET}回ほど叩いて音色を登録します`);setDetail("強弱を少し混ぜてください。各打音は約145msのPCMとして切り出し、環境ノイズを差し引いてスペクトル・MFCC・アタック・減衰を解析します。");
    }catch(e){console.error(e);setDetail(e?.message||"環境ノイズの収録に失敗しました");b.disabled=false;b.textContent="環境ノイズを収録"}finally{calBusy=false}
  }
  function startSampling(){
    if(calStage!=="sample")return;const b=ui("#micCalAction");b.classList.add("hidden");setCandidateMode("raw");setInstruction("パッドを続けて叩いてください");setDetail(`現在 0 / ${PAD_SAMPLE_TARGET}。音量ではなく音色を登録しています。強弱を混ぜてください。`);
  }
  function enterTestStage(){
    calStage="test";setPane("test","active");setCandidateMode("raw");ui("#micCalAction")?.classList.add("hidden");ui("#micCalStart")?.classList.remove("hidden");ui("#fpTestState").textContent="待機中";setInstruction("右の「叩いてみてください」で認識を確認してください");setDetail("テスト中も解析を続けます。高信頼の打音は登録モデルへ少しずつ取り込み、叩き方のばらつきに適応します。");
  }
  function testPaneFeedback(score,accepted){
    ...
    if(accepted){pane.classList.remove("hit");void pane.offsetWidth;pane.classList.add("hit");try{aecNode.port.postMessage({type:"suppressCandidates",ms:175})}catch{}if(typeof playDrum==="function"&&typeof DEFAULT_NOTE!=="undefined")playDrum(DEFAULT_NOTE.snare,"snare",.90)}
  }
  function showCalibration(){const s=ensureCalibrationScreen();setup.classList.add("hidden");s.classList.remove("hidden");resetCalibration();return new Promise(resolve=>{calResolve=resolve;ui("#micCalAction").onclick=()=>calStage==="noise"?void recordNoise():startSampling();ui("#micCalRetry").onclick=resetCalibration;ui("#micCalStart").onclick=()=>{if(!micCalibration||!padModel?.samples?.length)return;calibrationToken++;setCandidateMode("off");s.classList.add("hidden");calResolve?.(true);calResolve=null}})}
  ...
  function handlePadCandidate(m){
    ...
    if(calStage==="sample"){
      if(ui("#micCalAction")&&!ui("#micCalAction").classList.contains("hidden"))return;
      ...
      if(padModel.samples.length>=PAD_SAMPLE_TARGET){ui("#fpSampleState").textContent="登録完了";enterTestStage()}
      return;
    }
    if(calStage==="test"){
    ...
    if(runMode==="pad"&&typeof running!=="undefined"&&running&&!paused&&!document.body.classList.contains("acoustic-calibrating")&&performance.now()>=micSelfGuardUntil){
