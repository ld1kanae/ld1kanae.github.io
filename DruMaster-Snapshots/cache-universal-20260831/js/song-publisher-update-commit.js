...
(()=>{
  ...
  function openDb(){
    return new Promise((resolve,reject)=>{
      ...
      r.onerror=()=>reject(r.error||Error("更新セッションDBを開けませんでした"));
    });
  }
  ...
  async function putSession(value){
    const db=await openDb();
    return new Promise((resolve,reject)=>{
      const tx=db.transaction(STORE,"readwrite");tx.objectStore(STORE).put(value);
      tx.oncomplete=()=>{db.close();resolve()};tx.onerror=()=>{db.close();reject(tx.error||Error("更新セッションを保存できませんでした"))};
    });
  }
  ...
  async function apiGet(path,token){
    const r=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}?ref=${BRANCH}`,{headers:headers(token),cache:"no-store"});
    if(r.status===404)return null;
    if(!r.ok)throw Error(`GitHub API ${r.status}: ${path}`);
    return r.json();
  }
  async function putBlob(path,blob,sha,token,message){
    const body={message,content:await readBlob(blob),branch:BRANCH};if(sha)body.sha=sha;
    const r=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}`,{method:"PUT",headers:{...headers(token),"Content-Type":"application/json"},body:JSON.stringify(body)});
    if(!r.ok)throw Error(`GitHub upload ${r.status}: ${await r.text()}`);
    return r.json();
  }
  async function putText(path,text,sha,token,message){
    const body={message,content:encodeText(text),branch:BRANCH};if(sha)body.sha=sha;
    const r=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}`,{method:"PUT",headers:{...headers(token),"Content-Type":"application/json"},body:JSON.stringify(body)});
    if(!r.ok)throw Error(`GitHub update ${r.status}: ${await r.text()}`);
    return r.json();
  }
  async function deleteFile(path,sha,token,message){
    if(!sha)return;
    const r=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}`,{method:"DELETE",headers:{...headers(token),"Content-Type":"application/json"},body:JSON.stringify({message,sha,branch:BRANCH})});
    if(!r.ok)throw Error(`GitHub delete ${r.status}: ${await r.text()}`);
  }

  function regDoc(){try{return registerView.contentDocument}catch{return null}}
  function setUi(percent,stage,message,kind=""){
    const d=regDoc();if(!d)return;
  ...
  function applyVersions(draft,items,version){
    const changed=new Set(items.filter(x=>x.op!=="delete").map(x=>clean(x.path)));
    for(const stem of Object.values(draft.stems||{})){
      if(!stem?.path)continue;const p=clean(stem.path);if(changed.has(p))stem.path=`${p}?v=${version}`;
    }
    if(draft.midi){const p=clean(draft.midi);if(changed.has(p))draft.midi=`${p}?v=${version}`}
    if(draft.midiGzip){const p=clean(draft.midiGzip);if(changed.has(p))draft.midiGzip=`${p}?v=${version}`}
  }
  ...
  async function commitUpdate(msg){
    ...
    try{
      ...
      setUi(2,"GitHub更新開始","GitHubへ変更ファイルを反映しています…");
      ...
      const draft=structuredClone(session.draft||{}),version=Number(session.createdAt)||Date.now();
      draft.state="published";applyVersions(draft,items,version);
      ...
      for(let i=0;i<deletes.length;i++){
        const it=deletes[i];setUi(92+(i/Math.max(1,deletes.length))*6,"旧素材整理",it.label||it.path);
        await deleteFile(it.path,it.sha,token,`Remove song asset ${msg.id}: ${it.label||it.path}`);
      }

      session.draft=draft;
      session.mode="existing-edit";
      session.uploadItems=[];
      session.assetsCommittedAt=Date.now();
      ...
      console.error("Existing song update commit failed",e);
      setUi(0,"GitHub更新エラー",e?.message||String(e),"bad");
    }finally{active.delete(msg.sessionId)}
  }

  addEventListener("message",e=>{
    if(e.origin!==location.origin||e.source!==registerView.contentWindow)return;
    const d=e.data;if(d?.type!=="dm-song-editor-ready"||d.mode!=="update"||!d.id||!d.sessionId)return;
    void commitUpdate(d);
  });
})();
