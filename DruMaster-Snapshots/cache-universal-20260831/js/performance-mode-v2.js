...
(()=>{
  ...
  function gradeHit(note,delta){let mult,label;if(delta<=PERFECT_WINDOW){mult=1;label="PERFECT";counts.perfect++}else if(delta<=GREAT_WINDOW){mult=.75;label="GREAT";counts.great++}else{mult=.4;label="GOOD";counts.good++}score+=weight(note.type)*note.velocity/127*1000*mult;const node=document.querySelector("#score");if(node)node.textContent=String(Math.round(score)).padStart(6,"0");return label}
  ...
  function ensureOutputProbe(){
    ...
    if(typeof masterBus==="undefined"||!masterBus||typeof ac==="undefined"||!ac)return;
    outputAnalyser=ac.createAnalyser();outputAnalyser.fftSize=256;outputAnalyser.smoothingTimeConstant=.12;outputData=new Float32Array(outputAnalyser.fftSize);
  ...
  function readMicLevel(){return nodeLevel(micAnalyser,micData)}
  function readOutputLevel(){return nodeLevel(outputAnalyser,outputData)}
  ...
  function compensatedMic(raw,out,cal=micCalibration){
    ...
    return {rms:Math.max(0,raw.rms-predictedRms),peak:Math.max(0,raw.peak-predictedPeak),predictedRms,predictedPeak};
  }
  function stopMicLoop(){if(micRaf)cancelAnimationFrame(micRaf);micRaf=0}
  function releaseMic(){
    calibrationToken++;stopMicLoop();stopCalibrationBed();
    try{micSource?.disconnect()}catch{}try{micFilter?.disconnect()}catch{}for(const track of micStream?.getTracks?.()||[])track.stop();
  ...
  function ensureCalibrationScreen(){
    ...
    section.innerHTML=`<div class="mic-calibration-card">
      <p class="mic-cal-eyebrow">PAD PRACTICE</p><h2>マイク感度調整</h2>
      ...
      <p id="micCalCandidate" class="mic-cal-candidate"></p>
      <p id="micCalDetail" class="mic-cal-detail">段階ごとにボタンを押して測定します。勝手に次へ進みません。</p>
      <div class="mic-cal-actions manual-cal-actions">
        <button id="micCalAction" type="button">周囲の音を測定</button>
        <button id="micCalAccept" type="button" class="hidden">この値を採用</button>
        <button id="micCalStart" type="button" class="hidden" disabled>演奏開始</button>
        <button id="micCalRetry" type="button">最初からやり直す</button>
      </div>
    </div>`;
    app.appendChild(section);calibrationScreen=section;return section;
  }
  function ui(sel){return calibrationScreen?.querySelector(sel)}
  ...
  function setInstruction(text){const n=ui("#micCalInstruction");if(n)n.textContent=text}
  function setDetail(text){const n=ui("#micCalDetail");if(n)n.textContent=text}
  function setCandidate(text=""){const n=ui("#micCalCandidate");if(n)n.textContent=text}
  function setAction(text,disabled=false){const b=ui("#micCalAction");if(b){b.textContent=text;b.disabled=disabled;b.classList.remove("hidden")}}
  function showAccept(show){ui("#micCalAccept")?.classList.toggle("hidden",!show)}
  function showStart(show){const b=ui("#micCalStart");if(b){b.classList.toggle("hidden",!show);b.disabled=!show}}

  async function measureNoise(token,durationMs=1000){
    const rms=[],peak=[],until=performance.now()+durationMs;resetOutputHistory();
    while(token===calibrationToken&&performance.now()<until){const m=readMicLevel();rms.push(m.rms);peak.push(m.peak);updateCalibrationMeter(m.peak);await waitFrame()}
    if(token!==calibrationToken)throw Error("calibration-cancelled");
    return {rms:Math.max(.00005,percentile(rms,.65)),peak:Math.max(.0002,percentile(peak,.80))};
  }
  ...
  async function measureSpeakerRelation(noise,token,durationMs=1800){
    const rawRms=[],rawPeak=[],outRms=[],outPeak=[],ratioRms=[],ratioPeak=[],until=performance.now()+durationMs;resetOutputHistory();
    while(token===calibrationToken&&performance.now()<until){
      const m=readMicLevel(),o=readOutputLevel(),env=recentOutputEnvelope(o);rawRms.push(m.rms);rawPeak.push(m.peak);outRms.push(env.rms);outPeak.push(env.peak);updateCalibrationMeter(m.peak);
      if(env.rms>.0025)ratioRms.push(clamp(Math.max(0,m.rms-noise.rms)/env.rms,0,8));
      if(env.peak>.008)ratioPeak.push(clamp(Math.max(0,m.peak-noise.peak)/env.peak,0,8));
      await waitFrame();
    }
    if(token!==calibrationToken)throw Error("calibration-cancelled");
    if(ratioRms.length<5||ratioPeak.length<5)throw Error("端末音の測定量が不足しました");
    return {micRms:percentile(rawRms,.70),micPeak:percentile(rawPeak,.85),outputRms:percentile(outRms,.65),outputPeak:percentile(outPeak,.80),bleedRmsCoef:percentile(ratioRms,.60),bleedPeakCoef:percentile(ratioPeak,.60)};
  }
  function provisionalCal(){return calState?.speaker?{bleedRmsCoef:calState.speaker.bleedRmsCoef,bleedPeakCoef:calState.speaker.bleedPeakCoef}:null}
  async function captureOneHit(label,noise,token,timeoutMs=3200){
    const cal=provisionalCal(),until=performance.now()+timeoutMs;let prevR=0,prevP=0,capture=null;resetOutputHistory();
    while(token===calibrationToken&&performance.now()<until){
      const raw=readMicLevel(),out=readOutputLevel(),c=compensatedMic(raw,out,cal),now=performance.now();updateCalibrationMeter(raw.peak);
      if(capture){capture.rms=Math.max(capture.rms,c.rms);capture.peak=Math.max(capture.peak,c.peak);capture.rawPeak=Math.max(capture.rawPeak,raw.peak);if(now>=capture.until)return capture}
      else{
        const rise=c.rms-prevR,peakRise=c.peak-prevP,rmsGate=Math.max(.00028,noise.rms*1.55),peakGate=Math.max(.0011,noise.peak*1.50),riseGate=Math.max(.00010,noise.rms*.20),peakRiseGate=Math.max(.00028,noise.peak*.15);
        if((c.rms>rmsGate||c.peak>peakGate)&&(rise>riseGate||peakRise>peakRiseGate))capture={rms:c.rms,peak:c.peak,rawPeak:raw.peak,until:now+105};
      }
      prevR=c.rms;prevP=c.peak;await waitFrame();
    }
    if(token!==calibrationToken)throw Error("calibration-cancelled");return null;
  }
  function buildCalibration(){
    const {noise,speaker,strong,medium,weak}=calState,
          thresholdRms=Math.max(weak.rms*.85,noise.rms*1.65),thresholdPeak=Math.max(weak.peak*.85,noise.peak*1.55),
          riseGate=Math.max(.00008,(weak.rms-noise.rms)*.12,noise.rms*.12),peakRiseGate=Math.max(.00020,(weak.peak-noise.peak)*.10,noise.peak*.10),
          weakClear=weak.rms>noise.rms*1.35||weak.peak>noise.peak*1.35,ordered=strong.peak>medium.peak*1.04&&medium.peak>weak.peak*1.04;
  ...
  function nextHitStage(stage){return stage==="strong"?"medium":stage==="medium"?"weak":null}
  function hitLabel(stage){return stage==="strong"?"強く":stage==="medium"?"普通の強さで":"弱く"}
  function hitButton(stage){return stage==="strong"?"強を測定":stage==="medium"?"中を測定":"弱を測定"}
  function prepareStage(stage){
    calState.stage=stage;calState.candidate=null;showAccept(false);setCandidate("");
    if(stage==="noise"){setInstruction("周囲の音を測定します。パッドはまだ叩かないでください");setAction("周囲の音を測定")}
    else if(stage==="speaker"){setInstruction("端末からテスト伴奏を流して、マイクへの回り込みを測定します");setAction("端末音を測定")}
    else{setInstruction(`「${hitButton(stage).replace("を測定","")}」の測定を開始してから、パッドを${hitLabel(stage)}1回叩いてください`);setAction(hitButton(stage))}
  }
  function resetCalibrationUi(){
    for(const name of ["noise","speaker","strong","medium","weak"])setCalStep(name,"idle","待機");
    const marker=ui("#micCalThresholdMarker");if(marker){marker.classList.remove("show");marker.style.left="0%"}showStart(false);showAccept(false);setCandidate("");setDetail("段階ごとに操作して測定します。検出しただけでは次へ進みません。");prepareStage("noise");
  }
  function resetCalibration(){
    calibrationToken++;stopCalibrationBed();calBusy=false;micCalibration=null;calState={stage:"noise",noise:null,speaker:null,strong:null,medium:null,weak:null,candidate:null,stopBed:null};resetOutputHistory();resetCalibrationUi();
  }

  async function runCalibrationAction(){
    if(calBusy||!calState)return;const stage=calState.stage,token=calibrationToken;calBusy=true;setAction(stage==="noise"?"測定中…":stage==="speaker"?"測定中…":"検出待ち…",true);showAccept(false);setCandidate("");
    try{
      if(stage==="noise"){
        setCalStep("noise","active","測定中");setInstruction("約1秒、パッドを叩かずそのままにしてください");const noise=await measureNoise(token);if(token!==calibrationToken)return;calState.noise=noise;setCalStep("noise","done","完了");setDetail("周囲音を記録しました。次に端末音の回り込みを測定します。");prepareStage("speaker");
      }else if(stage==="speaker"){
        setCalStep("speaker","active","準備中");setInstruction("テスト伴奏を準備しています。パッドは叩かないでください");await loadCalibrationStems();if(token!==calibrationToken)return;startCalibrationBed();await waitMs(250);setCalStep("speaker","active","測定中");setInstruction("テスト伴奏を再生中です。まだパッドは叩かないでください");const speaker=await measureSpeakerRelation(calState.noise,token);if(token!==calibrationToken)return;calState.speaker=speaker;setCalStep("speaker","done","完了");setDetail("端末出力→マイク回り込みの係数を記録しました。以降は伴奏を流したまま打音を測定します。");prepareStage("strong");
      }else{
        setCalStep(stage,"active","検出待ち");setInstruction(`今から${hitLabel(stage)}1回だけ叩いてください`);const hit=await captureOneHit(stage,calState.noise,token);if(token!==calibrationToken)return;
        if(!hit){setCalStep(stage,"active","未検出");setDetail("打音を検出できませんでした。同じ段階の「測定」をもう一度押してください。");setAction(hitButton(stage));return}
        calState.candidate=hit;setCalStep(stage,"active","候補あり");setCandidate(`候補レベル  ${Math.round(meterPercent(hit.rawPeak))}%`);setInstruction("検出しました。この打音を使うなら「この値を採用」を押してください");setDetail("自動では次へ進みません。違う音を拾った場合は「測り直す」を押してください。");setAction("測り直す");showAccept(true);
      }
    }catch(e){if(String(e?.message)!=="calibration-cancelled"){console.error(e);setDetail(e?.message||"測定に失敗しました");setAction(stage==="noise"?"周囲の音を測定":stage==="speaker"?"端末音を測定":hitButton(stage))}}
    finally{calBusy=false;const b=ui("#micCalAction");if(b)b.disabled=false}
  }
  function acceptCandidate(){
    if(calBusy||!calState?.candidate)return;const stage=calState.stage;if(!["strong","medium","weak"].includes(stage))return;
    calState[stage]=calState.candidate;calState.candidate=null;setCalStep(stage,"done","採用");showAccept(false);setCandidate("");
    const next=nextHitStage(stage);
    if(next){setDetail(`${stage==="strong"?"強":"中"}の値を採用しました。次の強さを測定してください。`);prepareStage(next);return}
    micCalibration=buildCalibration();micNoiseFloor=micCalibration.noise.rms;stopCalibrationBed();resetOutputHistory();
    const marker=ui("#micCalThresholdMarker");if(marker){marker.style.left=`${meterPercent(micCalibration.thresholdPeak)}%`;marker.classList.add("show")}
    setInstruction("調整完了");setDetail(micCalibration.weakClear?"弱い打音の少し下を基準に設定しました。演奏中は端末の実際の出力レベルに応じて回り込み補正量が変化します。":"弱い打音と周囲音が近い状態です。必要なら最初からやり直してください。");ui("#micCalAction")?.classList.add("hidden");showStart(true);
  }

  function showCalibration(){
    const screen=ensureCalibrationScreen();setup.classList.add("hidden");screen.classList.remove("hidden");resetCalibration();
    return new Promise(resolve=>{
      calResolve=resolve;ui("#micCalAction").onclick=()=>void runCalibrationAction();ui("#micCalAccept").onclick=acceptCandidate;ui("#micCalRetry").onclick=resetCalibration;
      ui("#micCalStart").onclick=()=>{if(!micCalibration)return;calibrationToken++;stopCalibrationBed();screen.classList.add("hidden");calResolve?.(true);calResolve=null};
    });
  }
  ...
  function micFrame(){
    ...
    if(!paused){
      const raw=readMicLevel(),out=readOutputLevel(),c=compensatedMic(raw,out),rms=c.rms,peak=c.peak,rise=rms-micPrevRms,peakRise=peak-micPrevPeak,
            ...
            releasePeak=micCalibration?Math.max(micCalibration.noise.peak*1.30,peakGate*.52):Math.max(micNoiseFloor*1.60,peakGate*.52),now=performance.now(),
            loudEnough=rms>threshold||peak>peakGate,transient=rise>riseGate||peakRise>peakRiseGate,
            normalOnset=micArmed&&now>=micSelfGuardUntil&&loudEnough&&transient&&now-micLastHit>=MIC_REFRACTORY_MS,
            strongRetrigger=!micArmed&&now>=micSelfGuardUntil&&now-micLastHit>=MIC_RETRIGGER_GUARD_MS&&loudEnough&&(rise>riseGate*2.8||peakRise>peakRiseGate*2.8);
  ...
  startButton.onclick=async function(e){
    ...
    if(runMode==="pad"){
      ...
      try{await ensureMic();try{await ac.resume()}catch{}if(loadState)loadState.textContent=before;await showCalibration()}
      catch(err){console.error(err);if(loadState)loadState.textContent=err?.name==="NotAllowedError"?"パッド練習にはマイクの許可が必要です":(err?.message||"マイクを開始できません");setup.classList.remove("hidden");startButton.disabled=false;return}
      startButton.disabled=false;
    }
    const out=baseStart?await baseStart.call(this,e):undefined;
    if(runMode==="pad"&&typeof running!=="undefined"&&running)startMicLoop();else if(runMode==="pad"&&typeof running!=="undefined"&&!running)setup.classList.remove("hidden");return out;
  };
