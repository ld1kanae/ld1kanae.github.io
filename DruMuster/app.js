...
async function togglePause(forceResume=false){if(!running)return;if(!paused&&!forceResume){paused=true;cancelAnimationFrame(raf);await ac.suspend();$("#pausePanel").classList.remove("hidden");$("#pause").textContent="▶";$("#pause").setAttribute("aria-label","再生を再開")}else{await ac.resume();paused=false;$("#pausePanel").classList.add("hidden");$("#pause").textContent="Ⅱ";$("#pause").setAttribute("aria-label","再生を停止");loop()}}
