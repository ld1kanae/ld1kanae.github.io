...
const variants=['legacy','competition-open-state','competition-template','competition-hybrid'];
const root='drumscribe/experiments/generated-crash-competition-v55';
...
for(const variant of variants){
  ...
  for(const song of songs){
    ...
    console.log('DONE',variant,song,side?.adtofInfo?.cymbalPolicy?.crashCompetition||null);
  }
  await page.close();
}
await browser.close();
