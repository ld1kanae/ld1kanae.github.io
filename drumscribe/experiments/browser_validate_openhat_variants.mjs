...
const variants=['base','decay','gmd-rescue','decay-rescue','ride-open','ride-acoustic','ride-decay','ride-open-decay-rescue'];
...
for(const variant of variants){
  ...
  await page.goto('http://127.0.0.1:8000/drumscribe/?openHatVariant='+encodeURIComponent(variant),
  ...
  for(const song of songs){
    ...
    console.log('DONE',variant,song,timing?.adtofInfo?.openHat||null);
  }
  await page.close();
}
await browser.close();
