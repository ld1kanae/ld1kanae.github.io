...
def main():
        ...
        for lm in (.50,.80):
         for cb in (1.5,2.1):
          configs.append({"id":i,"hatThr":ht,"cymThr":ct,"scope":scope,"lmWeight":lm,
            ...
            "crashCymObs":1.0,"crashHead":.35,"beam":12});i+=1
    ...
    lag=aggregate(held)
    out={"schema":1,"description":"Ultra-constrained ADTOF candidate-grounded 16th-slot GMD decoder; pedal fixed; charts scoring-only.",
         "baseline":baseline,"top":rows[:40],"nestedLOO":{"aggregate":lag,"objective":objective(lag),"songs":loo}}
    (EXP/"results-adtof-gmd-tatum-metal-decoder-ultra.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
