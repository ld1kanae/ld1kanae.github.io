...
OUT=ROOT/"drumscribe/experiments/results-arrangement-kst-learned-v42.json"
MD=ROOT/"drumscribe/experiments/ARRANGEMENT_KST_LEARNED_V42.md"
MODEL_OUT=ROOT/"drumscribe/models/arrangement-kst/learned-rescore-v42.json"
    ...
    for song in SONGS:
        ...
        for group in GROUPS:
            for c in (sr.get("candidates") or {}).get(group,[]):
                ...
                # This experiment remains an A/A' rescue model; at least one
                # repeated-family occurrence must independently support the slot.
                if elig<1 or sup<1: continue
...
def fit_full_export(rows,algo):
    export={"schema":1,"version":"arrangement-kst-learned-v42","algorithm":algo,
...
def main():
    ...
    result={"schema":1,"date":"2026-09-23","experiment":"arrangement-kst-learned-v42",
    ...
    lines=[
      ...
      f"Candidate rows after A/A' support + post-filter-origin safety: **{len(rows)}**.",
      "",
