...
VARIANTS={
 'fullmix_w040_min3run4': dict(weight=.40,min3=4),
 'fullmix_w040_min3run6': dict(weight=.40,min3=6),
 'fullmix_w040_min3run8': dict(weight=.40,min3=8),
}
...
def infer(notes,phase,beat,duration,downs,extw,min3):
    f=event_features(notes,phase,beat,duration);n=len(f)
    allowed=(4,3) if downs else (4,)
    dp={(0,0,0):(0.,[])}
    for i in range(n):
        for (idx,last,run),(score,path) in [(k,v) for k,v in dp.items() if k[0]==i]:
            for L in allowed:
                switching=bool(path and L!=last)
                if last==3 and switching and run<min3:
                    continue
                j=i+L
                if j>=n:continue
                z=score+bar_score(f,i,L,CFG,phase,beat,downs,extw)
                if path:z+=CFG['run_bonus'] if L==last else -CFG['switch']
                if downs:z+=.35*extw*ext_support(phase+j*beat,downs,beat)
                newrun=run+1 if L==last else 1
                key=(j,L,newrun)
                if key not in dp or z>dp[key][0]:dp[key]=(z,path+[L])
    target=max(0,int((duration-phase)/beat))
    cand=[]
    for k,v in dp.items():
        if k[0]<max(0,target-6):continue
        if k[1]==3 and k[2]<min3:continue
        cand.append((abs(k[0]-target),-v[0],k,v))
    if not cand:
        cand=[(abs(k[0]-target),-v[0],k,v) for k,v in dp.items() if k[0]>=max(0,target-8)]
...
out={'schema':2,'description':'BeatThis fullmix-supported drum meter DP with minimum 3/4 run guard; chart scoring-only.','variants':{}}
for name,opt in VARIANTS.items():
    w=opt['weight'];min3=opt['min3']
    songs={};agg=[]
    for song in SONGS:
        notes,beat,phase,duration,ref,sigs,downs=inputs[song]
        pred,raw=infer(notes,phase,beat,duration,downs,w,min3)
    ...
    out['variants'][name]={'external_weight':w,'min3run':min3,'songs':songs,'summary':{
...
(ROOT/'drumscribe/experiments/results-fullmix-meter-fusion-v21.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
