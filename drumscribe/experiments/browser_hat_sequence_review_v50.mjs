...
const variants=['off','articulation','metal-grid','guarded-rescue'];
...
const output={schema:1,date:'2026-09-23',experiment:'hat-sequence-review-v50',predictionReadsReferenceMidi:false,variants,songs:{}};
for(const song of songs){
  ...
  output.songs[song]=await page.evaluate(async({song,variants})=>{
    ...
    return {bpm:tr.bpm,barPhaseSec:tr.barPhaseSec,numerator:tr.numerator||4,denominator:tr.denominator||4,rows};
  },{song,variants});
  console.log('DONE',song,Object.fromEntries(Object.entries(output.songs[song].rows).map(([k,v])=>[k,v.info])));
}
await browser.close();
await fs.writeFile('drumscribe/experiments/results-hat-sequence-predictions-v50.json',JSON.stringify(output,null,2)+'\n');
