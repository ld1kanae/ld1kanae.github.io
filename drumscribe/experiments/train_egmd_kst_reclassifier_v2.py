"""Train E-GMD kick/snare/tom candidate reclassifier v2 with hard negatives.
...
def candidate_rows(a,truth,hard_negatives=False):
    ...
    for g in GROUPS:
        ...
        for fr in pick_class(a,r,c,thr):
            t=fr/FPS
            out[g].append((t,feat(a,r,fr,c,thr),1 if near_truth(truth_by[g],t) else 0,float(a[fr,c]),float(r[fr,c])))
            seen.append(t)
        if hard_negatives:
            ...
            for t,other in truth:
                ...
                out[g].append((t,feat(a,r,fr,c,thr),0,float(a[fr,c]),float(r[fr,c])))
                seen.append(t)
    return out
...
def main():
    ...
    ap.add_argument("--output",default="drumscribe/models/egmd-kst-reclassifier-v2.json")
    ap.add_argument("--result",default="drumscribe/experiments/results-egmd-kst-reclassifier-v2.json")
    a=ap.parse_args()
    ...
    report={"schema":2,"dataset":"E-GMD v1.0.0","trainKits":train_kits,"heldOutKits":held_kits,
            "trainSequences":trseq,"validationSequences":vaseq,
            "trainManifest":train_manifest,"validationManifest":val_manifest,"classes":{}}
    deployment={"schema":2,"kind":"egmd-kst-candidate-logreg-hardneg","sampleRate":44100,"fps":FPS,
