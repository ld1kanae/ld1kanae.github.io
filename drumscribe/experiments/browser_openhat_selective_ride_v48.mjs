...
const configs=[
  ...
  {name:'selective55',variant:'ride-selective55-decay-rescue'},
  {name:'selective60',variant:'ride-selective60-decay-rescue'},
  {name:'selective65',variant:'ride-selective65-decay-rescue'},
];
const out={schema:1,date:'2026-09-23',experiment:'openhat-selective-ride-v48',
...
for(const song of songs){
  ...
  const row=await page.evaluate(async({song,configs})=>{
    const {transcribe}=await import('/drumscribe/transcribe.js?v=openhat-selective-v48');
...
await browser.close();
await fs.writeFile('drumscribe/experiments/results-openhat-selective-ride-predictions-v48.json',JSON.stringify(out,null,2)+'\n');
