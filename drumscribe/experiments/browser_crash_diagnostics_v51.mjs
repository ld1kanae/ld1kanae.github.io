...
const outDir='drumscribe/experiments/generated-crash-diagnostics-v51';
await fs.mkdir(outDir,{recursive:true});

const browser=await chromium.launch({headless:true});
const page=await browser.newPage();
page.setDefaultTimeout(20*60*1000);
await page.goto('http://127.0.0.1:8000/drumscribe/?cymbalVariant=legacy',{waitUntil:'domcontentloaded'});
for(const song of songs){
  ...
  console.log('DONE',song,side?.adtofInfo?.cymbalPolicy?.crashCandidates?.length||0);
}
await browser.close();
