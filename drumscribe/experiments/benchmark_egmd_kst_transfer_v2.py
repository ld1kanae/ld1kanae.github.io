"""Transfer-test E-GMD K/S/T hard-negative reclassifier v2 on DruMaster.
...
MODEL=json.loads((ROOT/"drumscribe/models/egmd-kst-reclassifier-v2.json").read_text())
...
def candidates(a):
    ...
    for g in GROUPS:
        ...
        for fr in pick(a,r,c,thr):
            out[g].append({
              "time":fr/FPS,"prob":predict(g,feat(a,r,fr,c,thr)),
...
def main():
    ...
    result={"schema":2,"description":"E-GMD KST hard-negative reclassifier transfer; DruMaster charts scoring-only.",
      "model":"models/egmd-kst-reclassifier-v2.json","policies":policies,"retained":retained}
    (EXP/"results-egmd-kst-transfer-v2.json").write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n")
