...
const out={schema:1,date:'2026-09-24',experiment:'hat-raw-acoustic-candidates-v63',
...
for(const song of songs){
  ...
  out.songs[song]=await page.evaluate(async song=>{
    const [{transcribe},{extractHatAcousticFeaturesV63}]=await Promise.all([
      import('/drumscribe/transcribe.js?v=hat-raw-v63'),
      import('/drumscribe/hat-context-v57.js?v=hat-raw-v63')
    ]);
...
await browser.close();
await fs.writeFile('drumscribe/experiments/results-hat-raw-acoustic-candidates-v63.json',JSON.stringify(out,null,2)+'\n');
