"""Train a compact E-GMD kick/snare/tom candidate reclassifier.
...
FEATURE_NAMES = (
    [f"act_{g}" for g in ("kick","snare","tom","hat","cymbal")] +
    [f"res_{g}" for g in ("kick","snare","tom","hat","cymbal")] +
    [f"mean5_{g}" for g in ("kick","snare","tom","hat","cymbal")] +
    [f"max5_{g}" for g in ("kick","snare","tom","hat","cymbal")] +
    ["target_prev2","target_prev1","target_next1","target_next2",
     "target_act_over_other","target_res_over_other",
     "target_act_over_low_threshold","target_res_over_low_threshold"]
)
...
def feat(a,r,fr,c,low_thr):
    T=len(a);lo=max(0,fr-2);hi=min(T,fr+3)
    cur=a[fr];res=r[fr]
    mean=a[lo:hi].mean(axis=0);mx=a[lo:hi].max(axis=0)
    def at(d):return float(a[max(0,min(T-1,fr+d)),c])
    other=max(float(np.max(np.delete(cur,c))),1e-5)
    rother=max(float(np.max(np.delete(res,c))),1e-5)
    vals=np.concatenate([cur,res,mean,mx]).astype(np.float32).tolist()
    vals += [
      at(-2),at(-1),at(1),at(2),
      float(cur[c])/other,float(res[c])/rother,
      float(cur[c])/max(low_thr,1e-5),float(res[c])/max(low_thr,1e-5)
    ]
...
def candidate_rows(a,truth):
    r=residual_matrix(a);out={g:[] for g in GROUPS}
    for g in GROUPS:
        c=CLASS_INDEX[g];thr=BASE_THRESH[g]*LOW_SCALE
        tt=[t for t,gg in truth if gg==g]
        for fr in pick_class(a,r,c,thr):
            t=fr/FPS
            out[g].append((t,feat(a,r,fr,c,thr),1 if near_truth(tt,t) else 0,float(a[fr,c]),float(r[fr,c])))
    return out
...
def kit_partition(rows):
    ...
    if len(kits)<8:raise RuntimeError(f"too few kits: {len(kits)}")
    # Spread selected kits across the 43-kit list, keeping train/held-out disjoint.
    train_idx=np.linspace(0,len(kits)-1,5).round().astype(int).tolist()
    hold_idx=np.linspace(3,len(kits)-4,4).round().astype(int).tolist()
    train=[];hold=[]
    for i in train_idx:
        if kits[i] not in train:train.append(kits[i])
    for i in hold_idx:
        if kits[i] not in train and kits[i] not in hold:hold.append(kits[i])
    # ensure exactly three held-out kits when possible
    for k in kits:
        if len(hold)>=3:break
        if k not in train and k not in hold:hold.append(k)
    return train[:4],hold[:3]
...
def collect(z,names,rows,model,processor,tag):
    ...
    for i,row in enumerate(rows,1):
        try:
            ...
            a=activities(model,processor,audio);cand=candidate_rows(a,notes);prod=production_events(a)
...
def choose_threshold(rows,probs,prod_metric,g):
    floor=prod_metric["precision"]-(.005 if g=="kick" else .02)
    best=None
    for thr in np.arange(.30,.951,.01):
        s=aggregate_candidate(rows,probs,float(thr),None,g)
        eligible=s["precision"]>=floor
        # favor F1; slight precision tie-break so transfer is conservative
...
def main():
    ...
    ap.add_argument("--output",default="drumscribe/models/egmd-kst-reclassifier-v1.json")
    ap.add_argument("--result",default="drumscribe/experiments/results-egmd-kst-reclassifier-v1.json")
    a=ap.parse_args()
    ...
    with RemoteZip(EGMD_ZIP) as z:
        ...
        trseq=choose_sequences(z,names,trg,6,4)
        vaseq=choose_sequences(z,names,vag,4,3)
    ...
    report={"schema":1,"dataset":"E-GMD v1.0.0","trainKits":train_kits,"heldOutKits":held_kits,
            "trainSequences":trseq,"validationSequences":vaseq,
            "trainManifest":train_manifest,"validationManifest":val_manifest,"classes":{}}
    deployment={"schema":1,"kind":"egmd-kst-candidate-logreg","sampleRate":44100,"fps":FPS,
