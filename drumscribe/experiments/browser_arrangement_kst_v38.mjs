...
const out='drumscribe/experiments/results-arrangement-kst-candidates-v38.json';
...
const result={
  schema:1,
  date:'2026-09-23',
  experiment:'arrangement-kst-candidates-v38',
  prediction_reads_reference_midi:false,
  note:'Reference chart.mid is not loaded in this browser stage. A/A-prime labels are structural families only.',
  modes,
  songs:{}
};
...
for(const song of songs){
  ...
  const row=await page.evaluate(async({song,modes})=>{
    const [{transcribe},{analyzeSections}]=await Promise.all([
      import('/drumscribe/transcribe.js?v=arrangement-kst-v38'),
      import('/drumscribe/arrangement/index.js?v=arrangement-kst-v38')
    ]);
