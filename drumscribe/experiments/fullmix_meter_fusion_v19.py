...
VARIANTS={
 'fullmix_support_040': .40,
 'fullmix_support_080': .80,
 'fullmix_support_120': 1.20,
}
...
def infer(notes,phase,beat,duration,downs,extw):
    f=event_features(notes,phase,beat,duration);n=len(f)
    # For this optional-fullmix branch, absent structural audio means stay with
    # incumbent 4/4 instead of hallucinating meter changes.
...
out={'schema':1,'description':'Drum meter DP plus BeatThis fullmix downbeat-head support when fullmix exists; chart scoring-only.','variants':{}}
for name,w in VARIANTS.items():
    songs={};agg=[]
    for song in SONGS:
        notes,beat,phase,duration,ref,sigs,downs=inputs[song]
        pred,raw=infer(notes,phase,beat,duration,downs,w)
    ...
    out['variants'][name]={'external_weight':w,'songs':songs,'summary':{
...
(ROOT/'drumscribe/experiments/results-fullmix-meter-fusion-v19.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
