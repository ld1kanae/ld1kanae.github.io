...
IN=EXP/'results-openhat-selective-ride-predictions-v48.json'
OUT=EXP/'results-openhat-selective-ride-v48.json'
MD=EXP/'OPENHAT_SELECTIVE_RIDE_V48.md'
...
def main():
    ...
    for song in SONGS:
        ...
        for cfg in data['configs']:
            ...
            per[name].append(r)
    report={'schema':1,'date':'2026-09-23','experiment':'openhat-selective-ride-v48',
    ...
    base=report['variants']['default']['summary']
    report['deltaVsDefault']={}
    for name,v in report['variants'].items():
        x=v['summary']
        report['deltaVsDefault'][name]={
          'hatMacroF1':x['hatMacroF1']-base['hatMacroF1'],
          'metalMacroF1':x['metalMacroF1']-base['metalMacroF1'],
          ...
          'kickF1':x['kick']['f1']-base['kick']['f1'],
          'snareF1':x['snare']['f1']-base['snare']['f1'],
          'tomF1':x['tom']['f1']-base['tom']['f1'],
        }
    OUT.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
    lines=['# Open-hat selective Ride→Open v48','',
      ...
      '|---|---:|---:|---:|---:|---:|---:|']
    for name in ('default','selective55','selective60','selective65'):
        x=report['variants'][name]['summary']
        lines.append(f"| {name} | {x['closed']['f1']:.6f} | {x['open']['f1']:.6f} | {x['ride']['f1']:.6f} | {x['hatMacroF1']:.6f} | {x['metalMacroF1']:.6f} | {x['hatRideOnset']['f1']:.6f} |")
    lines += ['','Deltas versus default:']
    for name in ('selective55','selective60','selective65'):
        d=report['deltaVsDefault'][name]
        lines += [f"- {name}: hat macro {d['hatMacroF1']:+.6f}; metal macro {d['metalMacroF1']:+.6f}; Open {d['openF1']:+.6f}; Ride {d['rideF1']:+.6f}; onset {d['hatRideOnsetF1']:+.6f}; K/S/T {d['kickF1']:+.6f}/{d['snareF1']:+.6f}/{d['tomF1']:+.6f}"]
    lines += ['','Guardrail: production adoption requires Open gain without material Ride/metal-macro regression and with K/S/T unchanged.','']
    MD.write_text('\n'.join(lines))
    print('\n'.join(lines))
