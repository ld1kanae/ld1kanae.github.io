...
export async function rescoreHatContextV54(decoded,events,variant='off'){
  if(variant==='off')return {events,info:{enabled:false,variant}};
  const valid=new Set(['hats50','hats55','selective55','binary55']);
  if(!valid.has(variant))return {events,info:{enabled:false,variant,error:'unknown variant'}};
  const samples=await monoAt44100(decoded),w=workspace();
  const threshold=variant==='hats50'?.50:.55;
  ...
  let changed=0,rideToOpen=0,rideToClosed=0,hatToOpen=0,openToClosed=0;
  const out=all.map(e=>{
    if(!['hat','open_hat','ride'].includes(e.group))return e;
    if((variant==='hats50'||variant==='hats55')&&e.group==='ride')return e;
    const p=probability(features(samples,e.time,nextMap.get(e),w));
    let group=e.group;
    if(e.group==='ride'){
      if(variant==='selective55'){if(p>=threshold){group='open_hat';rideToOpen++;}}
      else if(variant==='binary55'){group=p>=threshold?'open_hat':'hat';if(group==='open_hat')rideToOpen++;else rideToClosed++;}
    }else{
      group=p>=threshold?'open_hat':'hat';
      if(e.group!==group){if(group==='open_hat')hatToOpen++;else openToClosed++;}
    }
    if(group!==e.group)changed++;
    return {...e,group,hatContextProbability:p,hatContextRescored:group!==e.group};
  });
  return {events:out,info:{enabled:true,variant,threshold,changed,rideToOpen,rideToClosed,hatToOpen,openToClosed}};
}
