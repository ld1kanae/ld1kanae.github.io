...
## 2026-09-24: 音源別の採譜感度プロファイルをブラウザ保存

localStorageにファイル名単位で閾値倍率を自動保存し、同じファイル名を再読込した際に自動復元するUIを追加。値変更、1.0リセット、採譜完了時に保存し、採譜完了時にはノート総数/BPM/主要パート件数もメタ情報として更新する。個別削除ボタンあり。

別ファイルを選択した際は一度全倍率を1.0へ戻し、そのファイルの保存プロファイルだけを適用するため、直前の音源設定が未登録ファイルへ漏れない。静的GitHub Pagesからrepoへ直接書き込む認証系は追加していない。

## 2026-09-24: 閾値倍率レンジを0.0–2.0へ拡張

ユーザー要望により、採譜感度の全倍率を 0.50–1.50 から **0.0–2.0** へ拡張。step 0.05、default 1.0。0.0を内部で1.0へフォールバックしないよう各分類器の入力検証を修正し、ADTOF confidence計算にはゼロ除算ガードを追加。E-GMDの0閾値も ## 2026-09-24: Pedal HHをClosed HHへ出力統合
