...
(()=>{
  ...
  const DISPLAY_UPDATE_MS=1000/30;
  const OPEN_RAMP_BEATS=.25;
  let cachedNotes=null,cachedTiming=null,events=[];
  let displayedLevel=0,lastFrameMs=performance.now(),lastBeat=NaN,lastDomWriteMs=-Infinity,gaugeHeight=1;
  ...
  function refreshGaugeHeight(){
    const h=Number(gauge.clientHeight);
    if(Number.isFinite(h)&&h>0)gaugeHeight=h;
  }

  function writeLevel(level){
    const normalized=Math.max(0,Math.min(100,level));
    fill.style.setProperty("--hh-open-gauge-scale",(normalized/100).toFixed(5));
    fill.style.setProperty("--hh-open-gauge-opacity",(0.3+0.7*normalized/100).toFixed(3));
    const y=-(normalized/100)*gaugeHeight;
    handle.style.transform=`translate(-50%,50%) translate3d(0,${y.toFixed(3)}px,0)`;
  }
  ...
  function resetGauge(){
    ...
    lastDomWriteMs=-Infinity;
    writeLevel(0);
  }

  function smoothLevel(target,beat,now){
  ...
  function updateGauge(){
    ...
    if(now-lastDomWriteMs<DISPLAY_UPDATE_MS)return;
    lastDomWriteMs=now;
    ...
    const level=smoothLevel(baseLevel,beat,now);
    writeLevel(level);
  }

  handle.style.bottom="0";
  handle.style.willChange="transform";
  if(typeof ResizeObserver==="function")new ResizeObserver(refreshGaugeHeight).observe(gauge);
  requestAnimationFrame(()=>{refreshGaugeHeight();writeLevel(displayedLevel)});
