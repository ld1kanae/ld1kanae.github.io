...
(()=>{
  ...
  if(!document.querySelector('script[data-dm-update-guard]')){
    const s=document.createElement("script");s.src="js/song-publisher-update-guard.js?v=20260829-guard1";s.dataset.dmUpdateGuard="1";document.head.appendChild(s);
  }
  ...
  const launcher=tool=>`song-existing-edit.html?tool=${tool}&v=20260829-existing2`;
  ...
  function activate(which){
    for(const [key,tab] of Object.entries(tabs)){
      const active=key===which;tab.classList.toggle("active",active);tab.setAttribute("aria-selected",String(active));views[key].classList.toggle("active",active);
    }
  }
  function makeDirectTab(tab){
    tab.disabled=false;tab.classList.remove("locked");tab.classList.add("ready");tab.setAttribute("aria-disabled","false");const state=tab.querySelector(".tab-state");if(state)state.textContent="EDIT";
  }
  ...
  function loadEditors(d,token){
    const inherited={dmSongPublisher:{token:token||TOKEN_SENTINEL,repo:d.repo||"ld1kanae/ld1kanae.github.io",branch:d.branch||"main",id:d.id,sessionId:d.sessionId,at:d.at||Date.now()}};
  ...
  addEventListener("message",async e=>{
    ...
    if(d?.type==="dm-existing-editor-ready"&&(e.source===editorView.contentWindow||e.source===volumeView.contentWindow)&&d.id&&d.sessionId){
      session=d;if(!rememberedToken)rememberedToken=await readRememberedToken();loadEditors(d,rememberedToken);
      for(const tab of [editorTab,volumeTab]){const s=tab.querySelector(".tab-state");if(s)s.textContent="READY"}
      activate(d.tool==="volume"?"volume":"editor");return;
    }
    if(d?.type==="dm-song-editor-ready"&&e.source===registerView.contentWindow&&d.id&&d.sessionId){
      setTimeout(()=>loadEditors(d,d.token||rememberedToken),0);
      if(d.mode==="update")queueMicrotask(()=>activate("register"));
    }
  });

  ensureLaunchers();setTimeout(ensureLaunchers,250);
