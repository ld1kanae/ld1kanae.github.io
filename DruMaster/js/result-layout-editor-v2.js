...
(()=>{
  ...
  const KEY="drumasterResultLayoutEditorV2";
  ...
  function render(){
    ...
    if(device==="mobile"&&screen==="normal"){
      preview.insertAdjacentHTML("beforeend",'<i class="rl-guide v mid"></i><i class="rl-guide v left-center"></i><i class="rl-guide v right-center"></i>');
    }
  ...
  function buildList(){
    ...
    for(const [key,def] of Object.entries(M.defs[screen])){
      ...
      b.onclick=e=>{if(e.ctrlKey||e.metaKey){selected.has(key)?selected.delete(key):selected.add(key);if(!selected.size)selected.add(key)}else selected=new Set([key]);render()};
      list.appendChild(b);
    }
  }
  function mixed(prop){const vals=values().map(x=>x.v[prop]);return vals.length&&vals.every(v=>v===vals[0])?vals[0]:""}
  function fillInputs(){
    const ks=keys(),multi=ks.length>1;selectedTitle.textContent=multi?`${ks.length} ELEMENTS`:M.defs[screen][ks[0]]?.label||"SELECTED";selectionNote.textContent=multi?"複数選択中：ドラッグ・矢印キーでまとめて移動できます。":"Ctrl/Cmd＋クリックで複数選択できます。";
  ...
  function applyAbsolute(prop,value){for(const {key,v} of values()){v[prop]=value;if(["w","h","font"].includes(prop))v[prop]=Math.max(1,v[prop]);if(prop==="opacity")v[prop]=Math.max(0,Math.min(1,v[prop]));setValue(key,v)}render()}
  function applyDelta(dx,dy){for(const {key,v} of values()){v.x+=dx;v.y+=dy;setValue(key,v)}render()}
  for(const prop of ["w","h","font","line","letter","opacity"])inputs[prop].addEventListener("input",()=>{if(inputs[prop].value==="")return;applyAbsolute(prop,Number(inputs[prop].value))});
  for(const prop of ["x","y"]){
    inputs[prop].addEventListener("input",()=>{if(keys().length!==1||inputs[prop].value==="")return;applyAbsolute(prop,Number(inputs[prop].value))});
    inputs[prop].addEventListener("change",()=>{if(keys().length<2)return;const d=Number(inputs[prop].value)||0;if(d)applyDelta(prop==="x"?d:0,prop==="y"?d:0);inputs[prop].value=0});
  }
  preview.addEventListener("pointerdown",e=>{
    const el=e.target.closest(".rl-el");if(!el)return;e.preventDefault();const key=el.dataset.key;
    if(e.ctrlKey||e.metaKey){selected.has(key)?selected.delete(key):selected.add(key);if(!selected.size)selected.add(key);render();return}
    if(!selected.has(key))selected=new Set([key]);renderSelectionOnly();fillInputs();buildList();
    const starts={};for(const k of keys())starts[k]=M.obj(data[device][screen][k]);drag={id:e.pointerId,startX:e.clientX,startY:e.clientY,starts,axis:null};el.setPointerCapture(e.pointerId);
  });
  preview.addEventListener("pointermove",e=>{
    if(!drag||e.pointerId!==drag.id)return;let dx=(e.clientX-drag.startX)/scale,dy=(e.clientY-drag.startY)/scale;
    if(e.shiftKey){if(!drag.axis)drag.axis=Math.abs(dx)>=Math.abs(dy)?"x":"y";if(drag.axis==="x")dy=0;else dx=0}else drag.axis=null;
  ...
  function endDrag(e){if(!drag||e.pointerId!==drag.id)return;drag=null;render()}
  ...
  window.addEventListener("keydown",e=>{
    if(["INPUT","TEXTAREA"].includes(document.activeElement?.tagName)||!["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(e.key))return;e.preventDefault();const n=e.shiftKey?10:1;applyDelta(e.key==="ArrowLeft"?-n:e.key==="ArrowRight"?n:0,e.key==="ArrowUp"?-n:e.key==="ArrowDown"?n:0)
  });
  ...
  $("resetView").onclick=()=>{data[device][screen]=M.clone(M.base[device][screen]);selected=new Set([Object.keys(M.defs[screen])[0]]);render();toast("現在のビューを初期化しました")};
  $("resetAll").onclick=()=>{if(!confirm("4レイアウトすべて初期化しますか？"))return;data=M.clone(M.base);localStorage.removeItem(KEY);render();toast("すべて初期化しました")};
  function centerGroup(group,x0,x1){const items=group.map(k=>({k,v:M.obj(data.mobile.normal[k])}));if(!items.length)return;const min=Math.min(...items.map(i=>i.v.x)),max=Math.max(...items.map(i=>i.v.x+i.v.w)),cx=(min+max)/2,target=(x0+x1)/2,dx=Math.round(target-cx);for(const i of items){i.v.x+=dx;data.mobile.normal[i.k]=M.arr(i.v)}}
  function centerMobileColumns(){
    const left=["title","artist","scoreLabel","score","summary"];for(const k of left){const v=M.obj(data.mobile.normal[k]);v.x=Math.round((422-v.w)/2);data.mobile.normal[k]=M.arr(v)}
    const r=M.obj(data.mobile.normal.ranking);r.x=Math.round(422+(422-r.w)/2);data.mobile.normal.ranking=M.arr(r);centerGroup(["retry","home"],422,844);render();toast("スマホ左右カラムをセンタリングしました")
  }
  $("centerColumns").onclick=centerMobileColumns;
