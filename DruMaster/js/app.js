...
function loop(){if(!running||paused)return;const t=current();while(nextKick<notes.length&&notes[nextKick].time<=t){const n=notes[nextKick++];if(n.type==="kick"&&n.time>=t-.04){playDrum(n.note,n.type,n.velocity/127);n.hit=true;const e=$("#kickFx");e.classList.remove("hit");void e.offsetWidth;e.classList.add("hit")}}if(autoplay){let played=false;while(nextAuto<notes.length&&notes[nextAuto].time<=t){const n=notes[nextAuto++];if(n.type!=="kick"&&!n.hit){playDrum(n.note,n.type,n.velocity/127);n.hit=true;flashPart(PART[n.type]);played=true}}if(played)showJudge("AUTO")}else{let missed=false;while(missCursor<notes.length&&notes[missCursor].time<t-.18){const n=notes[missCursor++];if(n.type!=="kick"&&!n.hit){counts.miss++;missed=true}}if(missed)showJudge("MISS")}draw();if(t>=duration+.5)finish();else raf=requestAnimationFrame(loop)}
...
function setPauseTransport(state,label){const b=$("#pause");for(const n of [...b.childNodes])if(n.nodeType===Node.TEXT_NODE)n.remove();b.querySelectorAll(":scope > .transport-icon,:scope > .pause-transport-icon").forEach(n=>n.remove());b.dataset.transportIcon=state;b.setAttribute("aria-label",label)}
