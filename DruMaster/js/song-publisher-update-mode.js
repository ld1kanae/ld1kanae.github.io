...
(()=>{
  ...
  function syncAction(select,d=doc()){const input=$(select.dataset.input,d);if(!input)return;const replacing=select.value==="replace";input.disabled=mode==="update"&&!replacing;input.hidden=mode==="update"&&!replacing;if(!replacing)input.value="";updateSummaryHint(d)}
  ...
  function loadSong(id,d=doc()){
    ...
    const midiMeta=d.querySelector('.dm-update-meta[data-key="midi"]');if(midiMeta)midiMeta.textContent=`現在: ${(song.midi||`songs/${id}/chart.mid`).split("/").pop().split("?")[0]}`;setAction("midi","keep",d);
  ...
  async function runUpdate(d=doc()){
    ...
    try{
      ...
      const id=currentSong.id,title=$("title",d).value.trim(),artist=$("artist",d).value.trim(),duration=Number(currentSong.duration)||0,draft=clone(currentSong),stems=clone(currentSong.stems||{}),uploadItems=[],originals={};const changedMidi=actionFor("midi",d)==="replace";
      ...
      if(changedMidi){midiBlob=$("midi",d).files[0];midiAb=await midiBlob.arrayBuffer();const midiMeta=await apiGet(midiPath,t);uploadItems.push({path:midiPath,blob:new Blob([midiAb],{type:"audio/midi"}),sha:midiMeta?.sha||null,label:"chart.mid を更新"});const gz=await gzip(midiAb),gzipPath=`${ROOT}/${id}/chart.mid.gz`,gzipMeta=await apiGet(gzipPath,t);if(gz){uploadItems.push({path:gzipPath,blob:new Blob([gz],{type:"application/gzip"}),sha:gzipMeta?.sha||null,label:"chart.mid.gz を更新"});midiGzip=`songs/${id}/chart.mid.gz?v=${Date.now()}`}else{if(gzipMeta?.sha)uploadItems.push({op:"delete",path:gzipPath,sha:gzipMeta.sha,label:"chart.mid.gz を削除"});midiGzip=null}}
      else{midiBlob=await publicBlob(currentSong.midi||`songs/${id}/chart.mid`);midiAb=await midiBlob.arrayBuffer()}
      ...
      const sessionId=`${id}-update-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;setProgress(96,"編集準備",d);say("更新内容を編集セッションへ引き継いでいます…","",d);await storeSession({sessionId,id,createdAt:Date.now(),mode:"update",draft,originals,midiBlob:new Blob([midiAb],{type:"audio/midi"}),uploadItems});setProgress(100,"編集準備完了",d);say(changedMidi?"MIDIを差し替えました。Timing Correctionでタイミングを再確認してください。":"更新素材を準備しました。Timing Correctionで確認してください。","ok",d);parent.postMessage({type:"dm-song-editor-ready",token:t,repo:REPO,branch:BRANCH,id,sessionId,at:Date.now(),mode:"update"},location.origin)
