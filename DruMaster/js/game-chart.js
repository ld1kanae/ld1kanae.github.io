...
function rebuildHiHatGraphEnvelope(){
  ...
  for(const note of notes){
    ...
    actual.push({beat:Number(note.tick)/division,target:note.type==="hhOpen"?127:0});
  }
  actual.sort((a,b)=>a.beat-b.beat);
  hhGraphEvents=[];
  let previousBeat=-Infinity,previousTarget=0;
  for(let i=0;i<actual.length;i++){
    const event=actual[i],following=actual[i+1];
    ...
    const closing=event.target<previousTarget,
          rampStart=closing?(Number.isFinite(previousBeat)?Math.max(event.beat-HH_GRAPH_CLOSE_LEAD_BEATS,previousBeat):event.beat-HH_GRAPH_CLOSE_LEAD_BEATS):(Number.isFinite(previousBeat)?Math.max(event.beat-HH_GRAPH_OPEN_RAMP_BEATS,previousBeat):event.beat-HH_GRAPH_OPEN_RAMP_BEATS),
          rampEnd=closing?Math.min(rampStart+HH_GRAPH_CLOSE_RAMP_BEATS,following?.beat??Infinity):event.beat;
    hhGraphEvents.push({...event,from:previousTarget,rampStart,rampEnd,closing});
