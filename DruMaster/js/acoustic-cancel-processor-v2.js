class DruMasterAcousticCanceller extends AudioWorkletProcessor {
  constructor(){
    ...
    this.candidateFast=0;this.candidateSlow=0;this.candidateArmed=true;
  ...
  feedRapidCandidate(sample){
    ...
    const a=Math.abs(sample);this.candidateFast+=.28*(a-this.candidateFast);this.candidateSlow+=.006*(a-this.candidateSlow);
    const residual=this.candidateMode==='residual';
    const floor=residual
      ?Math.max(.00018,this.candidateNoise*.90,this.padProtect*.28)
      :Math.max(.00010,this.candidateNoise*.42);
    const trigger=residual
      ?Math.max(floor*1.08,this.candidateSlow*1.68)
      :Math.max(floor,this.candidateSlow*1.42);
    const release=residual
      ?Math.max(floor*.58,this.candidateSlow*1.08)
      :Math.max(floor*.52,this.candidateSlow*1.06);
    if(!this.candidateArmed&&this.candidateFast<release)this.candidateArmed=true;
    if(this.candidateMode!=='off'&&this.candidateArmed&&this.candidateFast>trigger&&a>floor*(residual?1:.72)){
  ...
  feedCandidate(sample){
    ...
    if(this.registrationMode)this.feedRegistrationCandidate(sample);else this.feedRapidCandidate(sample);
  }
  process(inputs,outputs){
    ...
    for(let i=0;i<n;i++){
      ...
      const level=Math.abs(err),lo=this.noiseRms*1.15,hi=this.noiseRms*4.5,target=level<=lo?.08:level>=hi?1:.08+.92*(level-lo)/(hi-lo),coeff=target<this.gate?.18:.055;
      this.gate+=coeff*(target-this.gate);const cleaned=err*this.gate;dst[i]=cleaned;for(let c=1;c<out.length;c++)out[c][i]=cleaned;
      ...
      this.feedCandidate(this.candidateMode==='raw'?d:cleaned);this.write=(this.write+1)&this.mask;
    }
