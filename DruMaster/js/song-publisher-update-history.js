...
(()=>{
  ...
  let boundDoc=null,stageObserver=null,modeObserver=null,current=null;
  const baselineById=new Map(),runtimeErrors=[],wrappedConsoles=new WeakSet();
  ...
  function readHistory(){try{const v=JSON.parse(localStorage.getItem(STORE_KEY)||"[]");return Array.isArray(v)?v:[]}catch{return []}}
  function writeHistory(items){try{localStorage.setItem(STORE_KEY,JSON.stringify(items.slice(0,MAX_HISTORY)))}catch{}}
  ...
  function consoleArg(v){if(v instanceof Error)return v.stack||v.message;if(typeof v==="string")return v;try{return JSON.stringify(v)}catch{return String(v)}}
  function pushRuntime(message){runtimeErrors.push(message);if(runtimeErrors.length>30)runtimeErrors.splice(0,runtimeErrors.length-30)}
  function bindConsoleCapture(target,prefix){
    ...
    c.error=(...args)=>{if(current)pushRuntime(`${prefix} console.error: ${args.map(consoleArg).join(" | ")}`);return original(...args)};
    wrappedConsoles.add(c);
  }

  normalizeInterrupted();bindConsoleCapture(window,"parent");
  if(document.documentElement.dataset.dmHistoryParentErrorBound!=="1"){
    ...
    addEventListener("error",e=>{if(current)pushRuntime(`parent error: ${e.error?.stack||e.message||"unknown"}`)});
    addEventListener("unhandledrejection",e=>{if(current){const r=e.reason;pushRuntime(`parent unhandledrejection: ${r?.stack||r?.message||String(r||"unknown")}`)}});
  }

  function ensureStyle(d){
    if(d.getElementById("dmUpdateHistoryStyle"))return;
    const s=d.createElement("style");s.id="dmUpdateHistoryStyle";s.textContent=`
      #dmUpdateHistory{display:none} body.dm-publisher-update #dmUpdateHistory{display:block}
      ...
      .dm-history-detail{min-width:0;color:#bac8d5;font-size:10px;line-height:1.55;overflow-wrap:anywhere}.dm-history-detail b{color:#edf5fb;font-weight:800}
      ...
      .dm-history-diag{border-top:1px solid #21313e;background:#050a0f;padding:9px}.dm-history-diag summary{cursor:pointer;color:#8fa2b4;font-size:9px;font-weight:800;letter-spacing:.06em}
      .dm-history-item.error .dm-history-diag summary{color:#ff9aa6}
      .dm-history-log{margin:8px 0 0;max-height:210px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;color:#91a6b8;font:9px/1.55 ui-monospace,SFMono-Regular,Consolas,monospace}.dm-history-item.error .dm-history-log{color:#e9a0aa}
  ...
  function ensurePanel(d){
    ensureStyle(d);let panel=d.getElementById("dmUpdateHistory");if(panel)return panel;
    const mode=d.getElementById("dmPublisherMode");if(!mode)return null;
  ...
  function formatTime(iso){try{return new Intl.DateTimeFormat("ja-JP",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(new Date(iso))}catch{return iso||""}}
  function render(d=doc()){
    if(!d)return;const panel=ensurePanel(d),list=d.getElementById("dmUpdateHistoryList");if(!panel||!list)return;
    ...
    for(const item of items){
      ...
      const detail=d.createElement("div");detail.className="dm-history-detail";const song=d.createElement("b");song.textContent=item.songTitle?`${item.songTitle} (${item.songId})`:item.songId||"不明な楽曲";detail.append(song,d.createTextNode(` — ${item.detail||"更新内容不明"}`));
      const status=d.createElement("span");status.className=`dm-history-status ${statusClass(item.status)}`;status.textContent=item.status||"UPLOADING";main.append(time,detail,status);row.appendChild(main);
      ...
      if(logs.length||item.error||item.diagnostics){
        ...
        const pre=d.createElement("pre");pre.className="dm-history-log";pre.textContent=[item.error?`ERROR: ${item.error}`:"",item.diagnostics||"",...logs].filter(Boolean).join("\n");diag.append(summary,pre);row.appendChild(diag);
      }
      list.appendChild(row);
    }
  }

  function cleanForStorage(item){const clean={...item};delete clean._midiBufferPromise;delete clean._midiHashPromise;return clean}
  function upsertHistory(item,d=doc()){
    const items=readHistory(),i=items.findIndex(x=>x.id===item.id),clean=cleanForStorage(item);if(i>=0)items[i]=clean;else items.unshift(clean);writeHistory(items);render(d);
  }
  function appendLog(tx,message,d=doc()){if(!tx)return;tx.logs=tx.logs||[];tx.logs.push(`[${nowIso()}] ${message}`);if(tx.logs.length>120)tx.logs=tx.logs.slice(-120);upsertHistory(tx,d)}
  ...
  function collectChanges(d){
    ...
    const midiFile=d.getElementById("midi")?.files?.[0]||null;
    if(midiFile&&!assetKeys.includes("midi")){parts.unshift(`MIDI: 差し替え (${midiFile.name||"ファイル"})`);assetKeys.push("midi")}
    if(baseline){
    ...
    }else if(now.order)parts.push(`表示順: ${now.order}`);
    return {detail:parts.length?parts.join(" / "):"メタデータ更新",assetKeys,includesMidi:assetKeys.includes("midi"),midiFile};
  }
  ...
  function parseMidiForGame(ab){
    ...
    if(str(4)!=="MThd")throw Error("MIDI header is not MThd");const header=u32(),format=u16(),tracks=u16(),division=u16();if(division&0x8000)throw Error("SMPTE MIDI is unsupported by game");if(header<6)throw Error(`Invalid MIDI header length: ${header}`);p=8+header;let drumNotes=0,totalNotes=0;
    ...
    if(!drumNotes)throw Error("Game parser found 0 drum notes on MIDI channel 10");return {format,tracks,division,drumNotes,totalNotes};
  }
  ...
  async function verifyGameLoad(tx,d){
    const expectedAb=tx._midiBufferPromise?await tx._midiBufferPromise:null,expectedBytes=expectedAb?.byteLength??null,expectedHash=tx._midiHashPromise?await tx._midiHashPromise:null;
    ...
    for(let i=0;i<VERIFY_RETRY_MS.length;i++){
      ...
      try{
        ...
        let meta=null,source="songs/registry.json";
        ...
        if(meta.midiGzip){
          ...
          if(typeof DecompressionStream==="function"){
            const gab=await new Response(gr.body.pipeThrough(new DecompressionStream("gzip"))).arrayBuffer(),gh=await hashBuffer(gab);parseMidiForGame(gab);if(gh!==actualHash)throw Error(`chart.mid.gz decompressed SHA mismatch: midi=${actualHash}, gzip=${gh}`);gzipResult=`OK (${gab.byteLength} bytes)`;
          }else gzipResult=`HTTP OK (${gr.headers.get("content-length")||"size unknown"})`;
        }
        const diag=["GAME LOAD CHECK: PASS",`songId: ${tx.songId}`,`metadata: ${source}`,`midi URL: ${meta.midi}`,`bytes: ${ab.byteLength}`,`sha256: ${actualHash}`,`MIDI format/tracks/PPQ: ${parsed.format}/${parsed.tracks}/${parsed.division}`,`drum notes: ${parsed.drumNotes}`,`midiGzip: ${gzipResult}`].join("\n");
        appendLog(tx,`GAME LOAD CHECK PASS: ${ab.byteLength} bytes / ${actualHash.slice(0,12)}… / ${parsed.drumNotes} drum notes`,d);return diag;
  ...
  function setVerifyBusy(d,on){for(const el of [d.getElementById("publish"),d.getElementById("dmConfirmUpdate"),d.getElementById("dmExistingSong")])if(el)el.disabled=!!on;d.querySelectorAll(".dm-mode-switch button").forEach(b=>b.disabled=!!on)}
  function setMainUi(d,stageText,logText,kind=""){const stage=d.getElementById("stage"),log=d.getElementById("log"),bar=d.getElementById("bar"),pct=d.getElementById("percent");if(stage)stage.textContent=stageText;if(log){log.textContent=logText;log.className=kind}if(bar)bar.style.width=stageText.includes("エラー")?"0%":"100%";if(pct)pct.textContent=stageText.includes("エラー")?"0%":"100%"}

  function startTransaction(d,stageText="更新開始"){
    ...
    if(file){tx._midiBufferPromise=file.arrayBuffer();tx._midiHashPromise=tx._midiBufferPromise.then(hashBuffer)}
    else{tx._midiBufferPromise=null;tx._midiHashPromise=null}
    runtimeErrors.length=0;current=tx;appendLog(tx,`START: ${changes.detail}`,d);appendLog(tx,`STAGE: ${stageText}`,d);return tx;
  }

  function finalizeSuccess(d,message){if(!current)return;current.status="SUCCESS";current.finishedAt=nowIso();current.stage="GitHub更新完了";current.error="";appendLog(current,`SUCCESS: ${message||"更新完了"}`,d);upsertHistory(current,d);captureBaseline(d);current=null}
  function finalizeError(d,message,diagnostics=""){
    ...
    current.status="ERROR";current.finishedAt=nowIso();current.stage="GitHub更新エラー";current.error=message||"不明なエラー";if(diagnostics)current.diagnostics=diagnostics;if(runtimeErrors.length)current.logs.push(...runtimeErrors.map(x=>`RUNTIME: ${x}`));appendLog(current,`ERROR: ${current.error}`,d);upsertHistory(current,d);current=null;
  }
  ...
  function onStageChange(d){
    const stage=safeText(d.getElementById("stage")?.textContent),log=safeText(d.getElementById("log")?.textContent),pct=safeText(d.getElementById("percent")?.textContent);if(!stage)return;
    if(FINAL_SUCCESS.has(stage)){
      if(!current)return;appendLog(current,`STAGE: ${stage} / ${log}`,d);if(current.includesMidi){void interceptSuccessForMidi(d);return}finalizeSuccess(d,log);return;
    }
    if(FINAL_ERROR.has(stage)){
      if(!current&&d.body.classList.contains("dm-publisher-update"))startTransaction(d,stage);if(current){appendLog(current,`STAGE: ${stage} / ${log}`,d);finalizeError(d,log||stage)}return;
    }
    if(stage==="ゲームロード確認")return;
    if(!IDLE.has(stage)&&stage!=="入力エラー"){
      const numeric=Number(pct.replace("%",""));if(!current&&(Number.isFinite(numeric)&&numeric>0||/確認|更新|UPLOAD|変換|保存|準備/.test(stage)))startTransaction(d,stage);else if(current&&current.stage!==stage){current.stage=stage;appendLog(current,`STAGE: ${stage}${log?` / ${log}`:""}`,d)}
    }
  }
  ...
  function bindRuntimeErrors(d){
    ...
    const record=(prefix,value)=>{if(!current)return;const msg=value instanceof Error?(value.stack||value.message):String(value||"");pushRuntime(`${prefix}: ${msg}`)};
    try{bindConsoleCapture(frame.contentWindow,"iframe")}catch{}try{frame.contentWindow.addEventListener("error",e=>record("iframe error",e.error||e.message))}catch{}try{frame.contentWindow.addEventListener("unhandledrejection",e=>record("iframe unhandledrejection",e.reason))}catch{}
  }

  function bind(d){
    if(!d?.head||!d.body)return false;ensurePanel(d);bindRuntimeErrors(d);if(boundDoc===d)return true;boundDoc=d;captureBaseline(d);
    d.getElementById("dmExistingSong")?.addEventListener("change",()=>scheduleBaseline(d));
    d.addEventListener("click",e=>{if(e.target?.id==="dmConfirmUpdate"&&d.body.classList.contains("dm-publisher-update")){startTransaction(d,"更新確認");appendLog(current,"CONFIRM CLICK: 入力ファイルを処理前にスナップショットしました。",d)}},true);
    stageObserver?.disconnect();stageObserver=new MutationObserver(()=>onStageChange(d));for(const el of [d.getElementById("stage"),d.getElementById("log"),d.getElementById("percent")])if(el)stageObserver.observe(el,{childList:true,subtree:true,characterData:true});
