(()=>{
  ...
  if(brand){
    fetch('assets/DruMaster.svg?v=20260828-mainlogosmooth1',{cache:'no-store'})
      .then(res=>{
...
/* Ranking synchronization is part of the shared web core. Keep the loader here
   as a safety net so Web, Android and Windows all execute exactly the same
   ranking client even when index.html is regenerated by older publisher files. */
(()=>{
  const load=(src)=>new Promise((resolve,reject)=>{
    if(document.querySelector(`script[data-dm-shared-src="${src}"]`))return resolve();
    const s=document.createElement('script');
    s.src=src;
    s.dataset.dmSharedSrc=src;
    s.onload=resolve;
    s.onerror=reject;
    document.head.appendChild(s);
  });
  load('js/ranking-sync.js?v=20260906-shared2')
    .then(()=>load('js/ranking-best-bridge.js?v=20260906-shared2'))
