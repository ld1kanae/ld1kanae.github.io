...
/* Ranking synchronization is part of the shared web core. Load every ranking
   component here with a new revision so previously cached bridge code cannot
   suppress legacy BEST migration. */
(()=>{
  const load=(src,key)=>new Promise((resolve,reject)=>{
    if(document.querySelector(`script[data-dm-shared-key="${key}"]`))return resolve();
    const s=document.createElement('script');
    s.src=src;
    s.dataset.dmSharedKey=key;
    s.onload=resolve;
    s.onerror=reject;
